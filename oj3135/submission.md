...
```text
Input:
โปรแกรมรับจำนวนเต็ม 3 ค่า คือ N K T
N คือจำนวนคนทั้งหมด
K คือจำนวนตำแหน่งที่ของขวัญถูกส่งต่อในแต่ละครั้ง
T คือหมายเลขของคนที่เป็นขโมย
Output:
โปรแกรมต้องพิมพ์จำนวนคนที่ได้พิจารณาของขวัญทั้งหมด
Constraints:
1 <= N <= 200,000
1 <= K <= N-1
1 <= T <= N
...
```text
Step1: อ่านค่า N, K และ T
Step2: หา gcd(N, K) เพื่อดูว่าของขวัญจะสามารถไปถึงคนกี่คนก่อนกลับมาคนที่ 1
Step3: ตรวจว่า (T - 1) % gcd(N, K) == 0 หรือไม่ เพื่อดูว่าขโมยอยู่ในเส้นทางที่ของขวัญจะผ่านหรือไม่
Step4: ถ้าของขวัญไปถึงขโมย ให้หาตำแหน่งของขโมยในลำดับการส่งของขวัญ และนับรวมขโมยด้วย
Step5: ถ้าของขวัญไม่ถึงขโมย ให้ตอบจำนวนคนที่ของขวัญผ่านก่อนกลับมาคนที่ 1
Step6: พิมพ์จำนวนคนที่ได้พิจารณาของขวัญทั้งหมด
```
...
```text
เหมือนวิธีแรกใ ช้ gcd(N, K) เพื่อหาจำนวนคนที่ของขวัญจะผ่านก่อนกลับมาคนที่ 1 จากนั้นตรวจว่าขโมยอยู่ในเส้นทางหรือไม่ ถ้าอยู่ จะใช้ Inverse เพื่อหาตำแหน่งของขโมยและนับจำนวนคนที่ได้ตรวจสอบจนถึงขโมย
```
