...
```text
Input:
บรรทัด1 : จํานวนเต็ม 2 ค่า num และ check (จํานวนร้าน และ จํานวนช่วงเวลาที่ต้องตรวจสอบ)
...
Constraints:
เปิดที่ start แต่ไม่นับที่ stop เช่น 600-660 เปิดถึงนาที 659
```
...
Step 2: วนอ่านเวลาเปิด-ปิดของแต่ละร้าน เก็บไว้เป็น list
Step 3: อ่านเวลาที่ต้องเช็คทั้งหมด
Step 4: สำหรับแต่ละเวลาที่ต้องเช็ค วนดูทุกร้านว่า start ≤ เวลานั้น < stop ไหม นับจำนวนร้านที่ตรงเงื่อนไข
Step 5: พิมพ์ผลลัพธ์ทั้งหมดคั่นด้วยช่องว่าง
```
...
```text
เหมือนแผนแรก เพราะเข้าใจง่าย เขียนง่าย
```
