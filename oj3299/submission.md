...
```text
Input:
รับ 2 จำนวนเต็มในบรรทัดเดียวกัน คั่นด้วยช่องว่าง: L (ความหนาของแถบ) และ N (จำนวนช่องที่ปลูกดอกไม้ไปแล้ว)
ขอบเขต: 1 ≤ L ≤ 10 และ 1 ≤ N ≤ 100,000
Output:
พิมพ์จำนวนเต็มหนึ่งค่า คือ หมายเลขแถบ ที่ช่องสุดท้าย (ช่องที่ N) ตกอยู่
Constraints:
1<=L<10,1<=N<=100000
```
...
```text
Step1: อ่านค่า L กับ N
Step2: ตั้ง total=0, k=0
Step3: เพิ่ม k ทีละ 1 บวกขนาดแนวทแยงของแถบ k (แนวทแยงที่ (k-1)L+1 ถึง kL) เข้า total
Step4: ถ้า total < N วนกลับ Step 3 ถ้าไม่ ให้หยุด
Step5: พิมพ์ k
```
